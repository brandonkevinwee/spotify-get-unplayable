...
public class UserAccess {
	...
	public JSONObject getUserProfile() throws IOException {
		...
		Request userDataRequest = new Request.Builder().url(userDataURL)
	...
	public List<JSONObject> getUserSavedTracks(String country) throws IOException {
		...
		do {
			HttpUrl userSavedTracksURL = HttpUrl.parse("https://api.spotify.com/v1/me/tracks").newBuilder()
					.addQueryParameter("market", market)
					.addQueryParameter("offset", Integer.toString(offset))
					.addQueryParameter("limit", Integer.toString(limit))
					.build();
			Request userSavedTracksRequestPage = new Request.Builder().url(userSavedTracksURL).addHeader("Authorization", "Bearer " + accessToken).build();
			Response userSavedTracksResponsePage = okHTTPClient.newCall(userSavedTracksRequestPage).execute();
