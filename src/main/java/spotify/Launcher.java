...
public class Launcher {
  ...
  public static void main(String[] args) throws Exception {
    ...
    String accessToken =
        tokenManager
            .getTokens(authorizationCode, TokenManager.GRANT_TYPE_AUTHORIZATION_CODE)
            .get("accessToken");
    ...
    List<JSONObject> listOfUnplayableTracks =
        listOfSavedTracks
            .stream()
            .filter(track -> !track.getJSONObject("track").getBoolean("is_playable"))
            .collect(Collectors.toList());
    listOfUnplayableTracks.forEach(
        track -> System.out.println(track.getJSONObject("track").getString("name")));
  }
}
